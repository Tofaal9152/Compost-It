...
function Menubar({
  ...
  return (
    <MenubarPrimitive.Root
      className={cn(
        'flex flex-row h-10 native:h-12 items-center',
        className
      )}
      {...props}
    />
...
function MenubarSubTrigger({
  ...
  const Icon = Platform.OS === 'web' ? Icons.ChevronRight : open ? Icons.ChevronUp : Icons.ChevronDown;
  return (
    ...
    >
      <View className='absolute left-2 flex h-3.5 w-3.5 items-center justify-center'>
        <MenubarPrimitive.ItemIndicator>
          <Icons.Check size={14} strokeWidth={3} className='text-foreground' />
        </MenubarPrimitive.ItemIndicator>
      </View>
      {children}
    </MenubarPrimitive.CheckboxItem>
